...
class UpdateCategorySchema(Schema):
    ...
    def __init__(self, category_obj, **kwargs):
        self.category = category_obj #set instance variable for current subj
    ...
    @validates("name")
    def validate_name(self, data):
        _category = Category.query.filter_by(name=data).first()
        if _category and _category != self.category:
            raise ValidationError("Name is taken.")

    @validates("code")
    def validate_code(self, data):
        _category = Category.query.filter_by(code=data).first()
        if _category and _category != self.category:
            raise ValidationError("Code is taken.")
